			...
			foreach ($releasebinincludefile in $releasebinincludefiles)
			{
				...
				if ($projectName -eq $releasebinincludefile.Name)
				{
					foreach ($fileitem in $filesitems)
					{
						...
						if ($fileitem.FileNameTo -eq "")
						{
							$filenametoitem = $fileitem.FileNameFrom
						}
						else
						{
						...
						try {
							New-Item -ItemType File -Path "$workingDir\Bin\$projectName\$filenamefromitem" -Force
						}
						catch { }
...
task ReleaseSrc -depends CleanWorking, UpdateVersion {
	...
	#build the robocopy exclusion folder list
	$robocopyXD = "";
	for ($i = 0; $i -lt $releaseSrcExcludeFolders.Count; $i++)
	{
		$robocopyXD += "`"" + $releaseSrcExcludeFolders[$i] + "`"";
		if ($i -lt $releaseSrcExcludeFolders.Count-1)
		{
			$robocopyXD += " ";
		}
	}
	#build the robocopy exclusion file list
	$robocopyXF = "";
	for ($i = 0; $i -lt $releaseSrcExcludeFiles.Count; $i++)
	{
		$robocopyXF += "`"" + $releaseSrcExcludeFiles[$i] + "`"";
		if ($i -lt $releaseSrcExcludeFiles.Count-1)
		{
			$robocopyXF += " ";
		}
	}
	...
	#copy src files
	robocopy $sourceDir $workingDir\Src /MIR /NP /XD .svn bin obj TestResults AppPackages $robocopyXD /XF *.suo *.user $robocopyXF | Out-Default
